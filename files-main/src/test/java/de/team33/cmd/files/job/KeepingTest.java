...
class KeepingTest {
    ...
    final void run_singlePath() throws RequestException {
        ...
        Keeping.job(condition(MUTE, "files", "keep", keepingPath.toString(), "jpg,jpe,jpeg", "tif,tiff")).run();
    ...
    final void run_dualPath() throws RequestException {
        ...
        ZipIO.unzip(Main.class, "zips/keeping.zip", keepingPath);
        Keeping.job(condition(MUTE, "files", "keep", path1, "none", "tif,tiff")).run();

        Keeping.job(condition(MUTE, "files", "keep", path1, "jpg,jpe,jpeg", path2, "tif,tiff")).run();
