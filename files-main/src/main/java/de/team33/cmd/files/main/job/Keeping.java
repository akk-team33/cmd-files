...
public class Keeping implements Runnable {
    ...
    public Keeping(final Context context, final String cmdName, final List<String> args) {
        this.context = context;
        this.cmdName = cmdName;
        this.args = String.join(" ", args);
        this.format = TextIO.read(Keeping.class, "Balancer.txt");
    }
    ...
    private static Runnable job(final Context context, final String cmdName, final List<String> args) {
        if (5 > args.size()) {
            return new Keeping(context, cmdName, args);
        } else {
